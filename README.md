...
语桥是一款面向轻中度失语症人群的日常表达辅助 App。项目目标不是替用户说话，也不是替代言语治疗师，而是在用户找词、理解对话或表达不准确时，提供低负担、可确认、可播报的辅助闭环。
...
```text
首次建立表达支持档案 / 用户卡住 / 对话中需要帮助 / 拍照识物 / 地点场景 / 词语训练
-> 结合症状档案、上下文、地点、个人物品、历史习惯生成候选或场景任务包
-> 用户选择或确认
-> 生成更清晰表达
-> 用户二次确认
-> TTS 播报或保存到本机记忆
...
- 小星星是“我现在想表达”的快捷中心，点击后打开“星语”面板。
- 星语第一屏提供 6 个高频表达，另有“更多表达”和“联系家人”入口。
- “更多表达”打开完整表达板，按身体不舒服、吃喝、如厕、求助、家人、医院、情绪、常用句等场景快速播报。
- “联系家人”支持配置家属姓名、电话和求助语，可播报求助句，也可通过系统拨号能力发起电话。
- App 底部为三页结构：左侧词语花园，中间首页，右侧我的。
- 首页功能切换浮窗类似辅助触控，可在各功能页快速切换到其他核心功能，并支持自动半隐藏。
- 视觉风格以浅色、莫兰迪色、半透明卡片、柔和高光为主，不再依赖第三方 `liquid_glass_renderer`。
- 已处理 Android 系统导航栏颜色，尽量让底部系统栏与 App 背景融合。
- 首次启动会进入“表达支持档案”，通过低负担问题记录当前困难、常用场景、提示偏好、训练时长、候选数量和家属协助需求。
- 完成首次档案后，系统会立即写入表达偏好，例如候选数量、图文模式、图标大小和是否开启个性化学习。

### 2. 表达偏好与低负担候选

- “表达偏好”支持候选数量、显示方式和图片/图标大小设置。
- 图标大小会真实影响补词和对话候选卡片，不只是保存设置值。
- 补词和对话的候选卡片以“图标 + 文字”为主，不直接展示语义分类，避免信息过载。
- 阅读困难或偏好图片的用户可以使用更大的图标；手部操作不便的用户可以通过更大图标和更大点击区域降低操作负担。
...
- 确认播报后会显示学习回执，让用户和评委能看到“语桥正在记住选择”。
...
- 已新增 `ScenePack` 场景任务包：地点识别后只显示轻提示，不强制跳转。
- 医院做深：提供“问诊辅助”，用户可点选症状、疼痛程度、持续时间、用药、过敏和诉求，确认后播报给医生。
- 医院场景还包含“下一步确认”，用于确认检查、用药、复诊或注意事项，避免听完医生后断在最后一步。
- 药店、购物、交通、家里、康复中心先做轻量表达包：点选候选，进入确认播报，再记录当前场景偏好。
- 场景页会显示“识别地点 -> 推荐表达 -> 确认播报 -> 记住习惯”的闭环，并在播报后给出“已学习当前场景”的回执。
...
- 星语、地点场景任务包和确认播报都会尽量回写到本地学习记录，让“越用越懂我”不只停留在补词页面。
- 训练记录会用于训练复习和掌握度统计，但不会直接把训练过的词强行提升为表达候选，避免把练习行为误当成真实表达意图。
...
- 左侧页面为“词语花园”，用于日常语言练习和词汇强化，不宣传为医学治疗或替代言语治疗师。
- 当前主体是找词训练：看图、听音、选四选一。
- 支持普通练习、常错复习、个人物品训练。
- 新增听理解训练：播放一句生活话语，让用户选择听到的物品、动作、地点或先后顺序。
- 听理解题目优先使用个人物品，其次使用现有词库和生活场景模板，例如“请把药盒拿给我”“我们现在去医院”“先吃药，再喝水”。
...
```text
lib/main.dart
  App 入口、首次表达支持档案、表达偏好、Qwen 服务、补词与成句流程、对话/拍照页面主体。

lib/star_home.dart
  三页主界面、小星星、功能球、底部导航、词语花园入口、地点场景入口和首页交互。

lib/star_speak_sheet.dart
  星语快捷表达、完整表达板、家属联系人配置、求助播报和系统拨号入口。
...
lib/scene_support/
  ScenePack 数据层和场景任务页面；医院为深问诊闭环，药店、购物、交通、家里、康复中心为轻量表达包。
...
lib/rehab_training.dart
  词语花园找词训练、听理解训练、掌握度、复习计划、训练结束反馈。
...
正式发布前建议使用后端代理，不把模型和地图服务 Key 打进 APK。当前客户端已支持：

```powershell
flutter run -d 2NP0224507003453 `
  --dart-define=YUQIAO_QWEN_PROXY_URL=https://你的服务/qwen/chat `
  --dart-define=YUQIAO_AMAP_PROXY_URL=https://你的服务/amap/regeo `
  --dart-define=YUQIAO_PARAFORMER_PROXY_WS_URL=wss://你的服务/asr/paraformer `
  --dart-define=YUQIAO_XFYUN_PROXY_WS_URL=wss://你的服务/asr/xfyun `
  --dart-define=YUQIAO_PROXY_TOKEN=可选短期Token
```

其中 Qwen 代理应兼容 DashScope/OpenAI chat completions 返回结构；高德代理应返回高德逆地理编码兼容 JSON；语音代理应保持对应 WebSocket 协议转发。
...
- 小星星不做复杂新功能，而是承担全局“快速开口”入口。
- 地点场景不做成每个地点一套重流程；医院做深，其他高频地点做轻量表达包，避免产品臃肿。
- 首次配置是“表达支持档案”，不是失语症类型诊断；它只用于选择更合适的界面和辅助方式。
- 康复训练定位为日常语言练习与辅助复习，不替代言语治疗师，不承诺医学疗效。
...
- 训练结果不应直接改变表达候选排序；真实表达中的确认、收藏、播报才是表达推荐的高价值信号。
...
- 词语花园继续打磨听理解题库：增加更多生活场景模板，但保持题目短、清楚、可重复播放。
- 读写训练可先作为轻量模式探索，例如看词选图、听词选字；组句训练暂不优先，避免增加不必要的认知负担。
